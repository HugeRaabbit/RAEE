...
def build_early_exit_table(
    ...
    gold_labels = [convert_token_to_id(tokenizer, label_word_mapping[str(example.label)]) for example in data]
    ...
    outputs = model.generate(**inputs, output_hidden_states=True, output_logits=True, return_dict_in_generate=True, max_length=2)
    hidden_states = outputs.encoder_hidden_states + outputs.decoder_hidden_states[-1][1:]
    embeddings = None
    if cls_layer is not None:
        embeddings = hidden_states[cls_layer].detach().cpu().numpy()[:, 0, :]
    assert len(hidden_states) == 2*num_layers + 1, f"The number of hidden layers ({len(hidden_states)}) does not match the model configuration ({2*num_layers + 1})"
    ...
    for i in range(2*num_layers + 1):
        ...
        if i < num_layers:
            hidden_state = model.transformer.encoder.final_layer_norm(hidden_state)
            hidden_state = model.transformer.encoder.dropout(hidden_state)
        elif i > num_layers and i < 2*num_layers:
            hidden_state = model.transformer.decoder.final_layer_norm(hidden_state)
            hidden_state = model.transformer.decoder.dropout(hidden_state)
        exit_logits = model.lm_head(hidden_state)
        exit_logits = exit_logits[torch.arange(exit_logits.size(0)), mask_pos] if i <= num_layers else exit_logits.squeeze()
...
def main():
    ...
    label_ids = [convert_token_to_id(tokenizer, token) for token in label_word_mapping.values()]
    ...
    logger.info("Example of the exit table: %s", exit_tables[0])
