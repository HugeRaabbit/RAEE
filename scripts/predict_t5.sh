...
for TASK in "${TASK_LIST[@]}"; do
    ...
    CUDA_VISIBLE_DEVICES=$DEVICE python $PROJECT_DIR/encoder_decoder/predict.py \
        --task_name $TASK \
        ...
        --nprobe $NPROBE \
        --seed $SEED \
        > $LOG_DIR/$TASK-t5-early-exit-results.log 2>&1
    ...
    CUDA_VISIBLE_DEVICES=$DEVICE python $PROJECT_DIR/encoder_decoder/predict.py \
        ...
        --mapping $MAPPING \
        --seed $SEED \
        > $LOG_DIR/$TASK-t5-origin-results.log 2>&1
done