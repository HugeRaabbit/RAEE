...
def main():
    ...
    gold_labels = [convert_token_to_id(tokenizer, label_word_mapping[str(example.label)]) for example in query_data]
    ...
    if args.do_exit:
        ...
        for exit_layer in exit_layer_set:
            ...
            for i in tqdm(range(num_batches), total=num_batches):
                ...
                mask_pos = [input_id.detach().cpu().numpy().tolist().index(tokenizer.mask_token_id) for input_id in inputs["input_ids"]]
            ...
            logger.info(f"Exit layer: {exit_layer}, avg forwarding time: {(end_time - start_time) / len(temp_data)}")
